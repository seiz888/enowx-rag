<#
.SYNOPSIS
    Remove the memgw ledger pull Scheduled Task.

.DESCRIPTION
    Removes exactly \memgw\ledger-pull, and only if its action still matches the
    recorded hash. Mirrors the other uninstallers and keeps each task's
    lifecycle independent.

    It deliberately does not delete D:\memgw-backups\vps\sealed: those sealed
    artefacts are the off-host copy of the authoritative ledger, and removing
    the task that fetches them is a different decision from destroying the ones
    that exist.
#>
...
$manifestPath = 'D:\memgw\run\installed-ledger-pull.json'
...
Write-Host 'memgw ledger-pull uninstall'
...
Write-Host 'note: the sealed artefacts in D:\memgw-backups\vps\sealed were NOT deleted.'
Write-Host 'done.'
